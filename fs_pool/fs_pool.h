...
/* 	Notes: chunk_size should be word size at the very least.
		   returns the size of total memory needed to hold all the requested chunks
	Time Complexity: O(1) */
size_t FSPoolCalcSize(size_t num_chunks, size_t chunk_size);


/* 	Notes: returns NULL pointer in case of fully allocated pool.
		   returns a pointer to a new chunk of memory of predetermined size
	Time Complexity: O(1) */
...
/* 	Notes: returns the number of free chunks left in the memory pool
	Time Complexity: O(n) */
